...
export const useBoardStore = defineStore('board', {
  ...
  actions: {
    ...
    async addCard(listId: string, title: string, status?: string, description?: string, priority?: string) {
      // 🎯 步驟1：找到目標列表
      ...
      // 🎯 步驟2：建立暫時卡片（立即顯示在 UI）
      // 使用時間戳作為暫時 ID，確保唯一性
      console.log('🏷️ [STORE] 建立暫時卡片，標題:', title, '列表 ID:', listId, '狀態:', status, '優先級:', priority, '描述', description)
      ...
      try {
        ...
        const realCard = await cardRepository.createCard(title, listId, description, status, priority)
